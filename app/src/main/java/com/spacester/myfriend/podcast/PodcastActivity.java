...
import com.spacester.myfriend.adapter.AdapterLiveChat;
import com.spacester.myfriend.live.StartLiveActivity;
import com.spacester.myfriend.model.ModelLiveChat;
...
public class PodcastActivity extends AppCompatActivity {
    ...
    public static final String LOGIN_MESSAGE = "com.spacester.myfriend.CHANNEL_LOGIN";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (type.equals("audience")){
            ...
            findViewById(R.id.camSwitch).setVisibility(View.GONE);
            findViewById(R.id.videoCam).setVisibility(View.GONE);
            findViewById(R.id.mic).setVisibility(View.VISIBLE);
        }

        findViewById(R.id.radio).setVisibility(View.VISIBLE);
        ...
        new Handler().postDelayed(() -> {
            FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
        ...
        if (type.equals("audience")){
            new Handler().postDelayed(() -> {
                FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (!snapshot.exists()){
                            channelProfile = 0;
                            Intent intent6 = new Intent(PodcastActivity.this, MainActivity.class);
        ...
        new Handler().postDelayed(() -> {
            FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).child("Users").addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {
        ...
        send.setOnClickListener(v -> {
            ...
            if (msg.isEmpty()){
                ...
                hashMap.put("msg", msg);
                FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).child("Chats").child(timeStamp).setValue(hashMap);
    ...
    private void readMessage() {
        DatabaseReference ref =  FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).child("Chats");
        ref.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                liveChat = new AdapterLiveChat(PodcastActivity.this, modelLives);
    ...
    protected void onDestroy() {
        ...
        if (channelProfile == Constants.CLIENT_ROLE_BROADCASTER){
            ...
            FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).child("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).removeValue();
            channelProfile = 0;
            Intent intent6 = new Intent(PodcastActivity.this, MainActivity.class);
            intent6.putExtra("hisUID", userId);
            intent6.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
    ...
    public void onBackPressed() {
        ...
        if (channelProfile == Constants.CLIENT_ROLE_BROADCASTER){
            ...
            FirebaseDatabase.getInstance().getReference().child("Podcast").child(channelName).child("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).removeValue();
            channelProfile = 0;
            Intent intent6 = new Intent(PodcastActivity.this, UserProfileActivity.class);
            intent6.putExtra("hisUID", userId);
            intent6.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
            startActivity(intent6);
            finish();
